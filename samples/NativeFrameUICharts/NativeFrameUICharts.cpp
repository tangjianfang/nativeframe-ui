...
public:
    ...
    // CP32: lets wWinMain seed the mode before create_charts wires the
    // palette into every chart view. Without this, --theme dark would
    // first paint light, then the click-toggle would have to fire.
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR cmd_line, int show_command) {
    ...
    // CP32: --theme seeds the palette before create_charts so the first
    // paint cycle already shows the requested mode.
