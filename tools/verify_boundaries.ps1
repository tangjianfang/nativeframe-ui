...
# Self-test fixture: this file legitimately contains forbidden markers as
# negative-test content for the boundary check itself; exclude it.
$selfTestFixture = Join-Path $rootPath 'tests/verify_boundaries_tests.ps1'

foreach ($item in $files) {
    if ($item.FullName -eq $selfTestFixture) {
        continue
    }
...
$allowedModuleDependencies = @{
    ...
    nfui_propertygrid = @('nfui_core', 'nfui_theme', 'nfui_control_base')
