...
`LineChartView` and `SplineChartView` use GDI+ `SmoothingModeAntiAlias` + `PixelOffsetModeHalf` for smooth line and curve rendering. The GDI+ session is process-lifetime, started by `nfui::initialize_chart_aa()` (typically called from `wWinMain` after `Application::initialize_common_controls()`). When the session is not active, both renderers fall back to the GDI `Polyline` / `PolyBezier` path so a missing call never breaks chart paint. `BarChartView` and `HBarChartView` keep pure GDI — their axis-aligned rects and rounded corners are already crisp.
